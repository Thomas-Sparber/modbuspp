...
int main(int argc, char **argv)
{
    ...
    vector<bool> read_coil = mb.modbus_read_coils(0, 1);

    // read input bits(discrete input)  function 0x02
    vector<bool> read_bits = mb.modbus_read_input_bits(0, 1);

    // read holding registers           function 0x03
    vector<uint16_t> read_holding_regs = mb.modbus_read_holding_registers(0, 1);

    // read input registers             function 0x04
    vector<uint16_t> read_input_regs = mb.modbus_read_input_registers(0, 1);
    ...
    vector<bool> write_cols = {true, true, true, true};
    mb.modbus_write_coils(0, write_cols);

    // write multiple regs              function 0x10
    vector<uint16_t> write_regs = {123, 123, 123, 123};
    mb.modbus_write_registers(0, write_regs);
